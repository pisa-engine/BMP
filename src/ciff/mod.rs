...
fn convert_to_bmp(input: &Path, output: &Path, bsize: usize, compress_range: bool, range_pruning_ration: f32) -> Result<()> {
    println!("{:?}", output);
    let mut ciff_reader =
    ...
    let mut builder: IndexBuilder;
    {
        ...
        builder = IndexBuilder::new(header.num_documents as usize, bsize,   range_pruning_ration);
